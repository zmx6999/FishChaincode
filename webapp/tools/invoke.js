...
// setup the fabric network
    ...
    var peer = fabric_client.newPeer('grpc://localhost:'+options.port);
...
// create the key value store as defined in the fabric-client/config/default.json 'key-value-store' setting
    Fabric_Client.newDefaultKeyValueStore({ path: store_path
        ...
        return fabric_client.getUserContext(options.username, true);
    }).then((user_from_store) => {
        if (user_from_store && user_from_store.isEnrolled()) {
            console.log('Successfully loaded '+options.username+' from persistence');
            member_user = user_from_store;
        } else {
            throw new Error('Failed to get '+options.username+'.... run registerUser.js');
        }
        ...
        if (isProposalGood) {
            ...
            let event_hub = channel.newChannelEventHub('localhost:'+options.port);
